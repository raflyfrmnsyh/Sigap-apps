...
@section('content')
    ...
    <div class="card p-5">
        <div class="backBtn">
            <a href="{{ route('pengaduan.ditanggapi') }}" class="btn btn-primary">
                <i class='bx bx-chevron-left'></i></a>
        </div>
        <div class="mt-3">
            <img src="../../../storage/{{ $data->foto }}" alt="" style="width:100%;object-fit:cover;height:420px;">
        </div>
    ...
    <div class="form-bx mt-3">
            <form action="/Admin/Pengaduan/tanggapan/edit{{ $data->id }}" class="formTanggapan" method="post">
                @csrf
                    <div class="card mb-4">
                      <h5 class="card-header">Form Edit Tanggapan</h5>
                      <div class="card-body">
                            
                        <div class="mb-3">
                            <select class="form-select" id="exampleFormControlSelect1" aria-label="Default select example" name="status"> 
                              <option selected>{{ $tanggapan->status }}</option>
                        ...
                        <div class="mb-3">
                            <textarea class="form-control" placeholder="Beri Tanggapan" id="exampleFormControlTextarea1" name="tanggapan" rows="3">{{ $tanggapan->tanggapan}}</textarea>
                        </div>
